...
export default function MobileEnquiryPage() {
  ...
  const GetEnquiryList = async () => {
    setShowLoader(true);
    try {
      const response = await axios.post('/api/customer-enquiry/select-mobile');
  ...
  return (
    ...
    {SuccessMsg && (<SuccessAlert />)}      
      <DashboardCard title="Customer enquiry mobile list">
            ...
            {EnquiryList && Array.isArray(EnquiryList) ? (              
              EnquiryList
                ...
                <TableRow key={EnquiryList.full_name} sx={{ border: "2px solid #f6f9fc" }}>
                    ...
                    <TableCell sx={{ borderRight: "2px solid #f6f9fc" }}>
                      <Typography>{EnquiryList.phone_number_1}</Typography>
                    </TableCell>
                    <TableCell sx={{ borderRight: "2px solid #f6f9fc" }}>
...
MobileEnquiryPage.getLayout = function getLayout(page) {
